...
main_inp = """nzydfxpc-rclop-qwzhpc-qtylyntyr-769[oshgk]
...
def day_4_2016_part_1(seq):
    """Part 1."""

    parts = seq.split('-')
    name = ''.join(parts[:-1])
    number = parts[-1]
    checksum = seq.split('[')[1].split(']')[0]
    name = sorted(name)
    letters = sorted(set(name), key=name.index)
    top = sorted(letters, key=lambda x: -name.count(x))[:5]
    if ''.join(top) == checksum:
        return True
    return False
...
@pytest.mark.parametrize('method, inp, expected', [
    (day_4_2016_part_1, 'aaaaa-bbb-z-y-x-123[abxyz]', True),
    (day_4_2016_part_1, 'a-b-c-d-e-f-g-h-987[abcde]', True),
    (day_4_2016_part_1, 'not-a-real-room-404[oarel]', True),
    (day_4_2016_part_1, 'totally-real-room-200[decoy]', False),
    (day_4_2016_part_2, 0, 0),
])
def test_day_4_2016_cases(method, inp, expected):
    """Test both parts."""
