...
def day_7_2017_part_1(seq):
    """
    Part 1.

    Easy to locate based on lack of references via simple string parsing.
    """
...
class Node(object):
    """Simple container for tree-like linked structure."""
    ...
    def search(self):
        """
        Depth-first search for an imbalance.

        Breaks and cascades up a boolean to indicate condition found.
        """
        # first, get a list of weights of all sub nodes
        w = []
        for sub_node in self.child:
            node_weight, sub_weights, was_found = sub_node.search()
            if was_found: # break if solution found by lower recursion level
                return node_weight, 0, True
            # otherwise, create weight array for this level
            w.append(node_weight + sum(sub_weights))

        # check if all weights are unbalanced
        if len(set(w)) > 1:
            # determine correct weight for this level
            freq = [w.count(w[i]) for i in range(len(w))]
            level_freq = max(freq)
            level_weight = w[freq.index(level_freq)]

            # from this, determine the source of unbalance
            for i in range(len(w)):
                if w[i] != level_weight:
                    bad_idx = i
                    break
            bad_node = self.child[i]

            # look at and past this node for one that is unbalanced, but has
            # balanced children. That's the target for part 2 of this problem.
            nw, sw, was_found = bad_node.search()
            if len(set(sw)) == 1:
                # found it - return corrected weight, and indicate it to any
                # higher calling stack frames that we're done, and pass the
                # solution up.
                return level_weight - sum(sw), None, True

        # return weights
        return self.weight, w, False
...
def day_7_2017_part_2(seq):
    ...
    Harder. Build up linked structure to capture directed graph. Then:
    1. Recurse to locate position of imbalance.
    2. Find deepest child with a correct balance.
    3. Return adjusted weight.
    """
    # Capture all nodes in a linear dict structure, indexed by name.
    # Save all connection information as lists of string names for now.
    seq_list = seq.split('\n')
    nodes = {}
    for step in seq_list:
        ...
        step = step.split("->")
        if len(step) > 1:
            connected = step[1].split(",")
            nodes[name].child_names = [i.strip() for i in connected]

    # find and set the root node for the graph
    root_name = day_7_2017_part_1(seq)
    bottom = nodes[root_name]

    # Next, use the connections to build up a linked structure. Will naturally
    # be topologically sorted, with bottom = the actual root node.
    for name in nodes.keys():
        this_node = nodes[name]
        for subnode in this_node.child_names:
            this_node.child.append(nodes[subnode])
    # all nodes are now reference linked on the heap via their class
    # attribute `child` (list).

    # Recurse to find the position of imbalance, and the corrected weight
    corrected_weight, _, _ = bottom.search()
...
if __name__ == '__main__':
    # show solution
    print('day_7_2017_part_1 solution:', day_7_2017_part_1(main_inp))
