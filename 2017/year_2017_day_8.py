...
def day_8_2017_part_1(seq):
    ...
    for line in lines:
        ...
        if 'dec' in line:
            mult = -1
            oper = 'dec'
        name, line = line.split(oper)
        name = name.strip()
        registers[name] += 0
        amount, line = line.split('if')
        amount = int(amount.strip())
        cond = line.split()[0].strip()
        mix = 'registers["%s"]' % cond
        eval_string = mix + ''.join(line.split()[1:])
        if eval(eval_string):
            registers[name] += mult*amount

    return max([registers[i] for i in registers])


# Part 2 --------------------------------


def day_8_2017_part_2(seq):
    """Part 2."""
    max_val = 0
    registers = defaultdict(int)
    lines = seq.split("\n")
    for line in lines:
        ...
        if eval(eval_string):
            ...
            if registers[name] > max_val:
                max_val = registers[name]
...
test_1 = """b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10"""
@pytest.mark.parametrize('method, inp, expected', [
    (day_8_2017_part_1, test_1, 1),
    (day_8_2017_part_2, test_1, 10),
])
def test_day_8_2017_cases(method, inp, expected):
    """Test both parts."""
